 ...
 * @author Ilana Shapiro: modified the constructor and added the keyPressed method so the user can make the "zebra" green square jump by pressing the space bar
 */
...
public class ActorWorld extends World<Actor>
{
    ...
    /**
     * Constructs an actor world with a default grid, and initializes the zebra object.
     */
    public ActorWorld(Zebra z)
    {
		super();
		zebra = z;
    }
    ...
    public void step()
    {
		...
		for (Actor a : actors)
        {
            ...
            if (a.getGrid() == gr && (a.canMove() == false))
            {
            	canAct = false;
    ...
    public void add(Location[][] loc, Actor[][] occupant)
    {
    	for(int r = 0; r < loc.length; r++)
        {
        	...
        	{
        		occupant[r][c].putSelfInGrid(getGrid(), loc[r][c]);
        	}
        }
    }

    public void add(Location[][] loc, SideScrollerArray arr)
    {
    	SideScroller[][] array = arr.getObj();
    	loc = arr.getLoc();
    	...
    	for(int r = 0; r < loc.length; r++)
        {
        	for(int c = 0; c < loc[0].length; c++)
        	{
        		array[r][c].putSelfInGrid(getGrid(), loc[r][c], array[r][c]);
        	}
        }
    }
